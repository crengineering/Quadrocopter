function cs = code_config()
% MATLAB function for configuration set generated on 15-Jul-2026 17:16:45
% MATLAB version: 25.2.0.2998904 (R2025b)

cs = Simulink.ConfigSet;

% Original configuration set version: 25.0.0
if cs.versionCompare('25.0.0') < 0
...
cs.set_param('Name', 'code_config'); % Name
...
cs.set_param('ArtificialAlgebraicLoopMsg', 'warning');   % Artificial algebraic loop occurrences not eliminated
...
cs.set_param('OperatingPointInterfaceChecksumMismatchMsg', 'warning');   % Operating point interface checksum mismatch
cs.set_param('OperatingPointContentsChecksumMismatchMsg', 'error');   % Operating point contents checksum mismatch
...
cs.set_param('SFEntryPathToHistoryDiag', 'error');   % Invalid default or entry path
...
cs.set_param('ModelReferenceMinAlgLoopOccurrences', 'on');   % Minimize artificial algebraic loop occurrences
...
cs.set_param('SimDynamicStringBufferSize', 256);   % Buffer size of dynamically-sized string (bytes)
...
cs.set_param('SubcomponentCodeInterfacePackaging', 'Reusable function');   % Code interface packaging
