...
**ASPICE:** SYS.3 supporting analysis — propeller → sample-rate path · realizes SYS-MEC-002, SYS-MEC-004, SYS-MEC-006, SYS-TIM-001 · process: QuadSE/requirements/README.md

**Decided, 2026-08-30 (R-012 gate 1).** The propeller ambiguity this note
...
**not yet measured** — SYS-MEC-004's bench sweep is blocked on the actuation
path (SYS-ACT-001) — so `quad_params.m`'s numeric fields are unchanged
...
2026-08-30 (flight-control, R-012 gate 2) against the decided drive and the
corrected SYS-MEC-006.
...
prop) used to describe two live candidates. R-012 gate 1 (2026-08-30) fixed
...
| AUM (incl. 10 m airborne tether) | **1,77–1,82 kg** (T/W 2,20–2,26) — dispatch/R-012.md §4 |

**`kT` has no measurement yet** for this exact pair — SYS-MEC-004's bench
sweep needs the actuation path (SYS-ACT-001), which doesn't exist. The only
...
thrust table predicts (dispatch/R-012.md §4a, §6).
...
## 3a. SYS-MEC-006 — adjudicated for this drive, 2026-08-30

SYS-MEC-006 was corrected the same day: the sufficient no-aliasing case
...
flight-control** (SYS-MEC-006's own wording); here it is, against the
decided drive:

- **No-load bound** (worst case, never reached under load): KV880 at a
  fully-charged 4S pack (16,8 V) ⇒ 14 784 rpm ⇒ 246 Hz rotor ⇒ **493 Hz
  blade-pass** (dispatch/R-012.md §2). This clears the raw `< 507 Hz` target
...
- **Realistic loaded full-throttle** is well below the no-load bound — a
  ...
  (SYS-MEC-004), but by the same physics cited in the SYS-MEC-006 change log
...
**Judgement: SYS-MEC-006 is satisfied by design for this drive under both
...
set `verified` — that needs the measured `w_max` from SYS-MEC-004, which is
blocked on SYS-ACT-001 — but nothing here argues for holding the firmware
...
**measurement**, blocked on SYS-ACT-001. When the sweep exists (see
SWR-MDL-003, dispatch/R-012.md §6 for the rig and fit method):

1. **Fit `kT`, `kQ`, `tau`, `w_max`** from the bench sweep with the
   coefficient-fit script (SWR-MDL-003) and put them in `quad_params.m:8-11`.
   Set the real `m` at `:2` from the SYS-MEC-003 weigh-in once the frame and
...
2. **Re-run the arithmetic:**
   ...
   ```
3. **Check `f_blade_max` by the SYS-MEC-006 criterion (§3a), not the raw
   Nyquist bound alone:**
...
|---|---|---|---|
| `:2` | `m` | SYS-MEC-003 weigh-in table (dispatch/R-012.md §4), once the frame (400 g estimate) and breakout PCB (100–150 g estimate) are weighed, not planning values | evidence/INDEX.md row + dated comment citing it |
| `:8` | `kT` | SWR-MDL-003 fit script output on the bench sweep | evidence/INDEX.md row (MF4 of the sweep) + fit script's reported R²/residual in the comment |
